...
class AltasibFeedbackCRM
{
        function AddLead($arFields)
        {
                ...
                foreach(GetModuleEvents("altasib.feedback", "OnBeforeSendLead", true) as $arEvent)
                {
                        if(ExecuteModuleEventEx($arEvent, array(&$arFields)) == false)
                        {
                                if(!$APPLICATION->GetException())
                                        $APPLICATION->ThrowException(GetMessage("ALTASIB_FEEDBACK_LEAD_ERR", array("#HANDLER#"=>$arEvent["TO_NAME"])));
                                return false;
                        }
                }
