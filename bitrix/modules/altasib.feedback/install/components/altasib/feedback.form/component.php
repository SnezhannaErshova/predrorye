...
if (!function_exists('altasib_fb_check_spec_chars')) {
    function altasib_fb_check_spec_chars($val, $aParams)
    {
        if ($aParams['SPEC_CHAR'] == 'Y') {
            $chars_array = explode(",", $aParams['SPEC_CHAR_LIST']);
...
if ($obCache->InitCache(86400, serialize(array($arFilter, $arParams["IBLOCK_ID"], "iblock", $arParams['FORM_ID'])), $cachePath)) {
...
$obCache = new CPHPCache();
if ($obCache->InitCache(3600, serialize(array($arPFiltr, $arParams['FORM_ID'], "section")), $cachePath)) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["FEEDBACK_FORM_" . $ALX] && $arParams["SECTION_FIELDS_ENABLE"] == "Y" && $_POST["REFRESH"] == "Y") {
    ...
    foreach ($arParams["PROPERTY_FIELDS_REQUIRED"] as $k => $v) {
        if (in_array($v, $arParams["PROPERTY_FIELDS"])) {
            if (is_array($_POST["codeFileFields"])) {
    ...
    if (($arParams['USER_CONSENT'] == 'Y') && (intval($arParams["USER_CONSENT_ID"]) > 0)) {
    ...
    // requare select need
    if (($arParams['REQUIRED_SECTION'] == "Y") && (!empty($arResult["TYPE_QUESTION"]))) {
    ...
    if ($arParams["USE_CAPTCHA"]) {
        if ($arParams["CAPTCHA_TYPE"] == "recaptcha") // Google reCAPTCHA v2
        {
        ...
        } elseif ($arParams["CAPTCHA_TYPE"] == "recaptchav3") // Google reCAPTCHA v3
        {
            if (COption::GetOptionString('altasib.feedback', 'ALX_COMMON_CRM') == "Y") {
                $site_key = COption::GetOptionString('altasib.feedback', 'ALX_RECAPTCHA_SITE_KEY');
                $server_key = COption::GetOptionString('altasib.feedback', 'ALX_RECAPTCHA_SECRET_KEY');
            ...
            if (!$answers["success"]) {
                ...
                if ($answers["score"] < floatval($arParams["RECAPTCHA_VALID_SCORE"])){
                    $strCaptchaErr = '';
                    $strCaptchaErr .= GetMessage("ALX_CP_WRONG_RECAPTCHA_SCORE");
                    $arResult["FORM_ERRORS"]["CAPTCHA_WORD"]["ALX_CP_WRONG_CAPTCHA"] .= $strCaptchaErr;
                }
            }
...
$obCache = new CPHPCache();
if ($obCache->InitCache(86400, serialize(array($arPFiltr, $arParams["PROPERTY_FIELDS"], "property", $arParams['FORM_ID'])), $cachePath)) {
...
foreach ($arProp as $prop) {
    ...
    if ($prop["PROPERTY_TYPE"] == "L") {
        ...
        if ($obCache->InitCache(86400, serialize(array($prop["ID"], "PropertyEnum", $arParams['FORM_ID'])), $cachePath)) {
    ...
    if ($prop["PROPERTY_TYPE"] == "E") {
        ...
        if (isset($prop["LINK_IBLOCK_ID"])) {
            ...
            if ($obCache->InitCache(3600, serialize(array($arFilter, "LinkElements", $arParams['FORM_ID'])), $cachePath)) {
    ...
    if ($prop["PROPERTY_TYPE"] == "G") {
        ...
        if (isset($prop["LINK_IBLOCK_ID"])) {
            ...
            if ($obCache->InitCache(3600, serialize(array($arFilter, "LinkSections", $arParams['FORM_ID'])), $cachePath)) {
...
if ($arParams["USE_CAPTCHA"] == "Y" && ($arParams["CAPTCHA_TYPE"] == "recaptcha" || $arParams["CAPTCHA_TYPE"] == "recaptchav3")) {
